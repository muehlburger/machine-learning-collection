...
print(lineToTensor('0.19699140401146104,0,1,0,0,0,1.0,0,0.0353535353535354,0,0,0,1,0.0698119025593586,0.10824742268041199,0,0,0,1,0,0,0').size())
...
h0 = torch.zeros(n_hidden, 1, )
c0 = torch.zeros(1, n_hidden)
...
for i in range(n_confusion):
    category, line, cateogry_tensor, line_tensor = randomTrainingExample()
    output = evaluate(line_tensor)
    guess, guess_i = categoryFromOutput(output)
    category_i = all_categories.index(category)
    confusion[category_i][guess_i] += 1

# Normaliyze by dividing every row by its sum
